...
export abstract class ChangeBuilderBase {
  ...
  alterType(column: string, newType: string) {
    return `ALTER COLUMN ${this.wrapIdentifier(column)} TYPE ${this.wrapLiteral(
      newType
    )}`;
  }
  ...
  alterDefault(column: string, newDefault: string | boolean | null) {
    if (newDefault === null) {
      ...
      return `ALTER COLUMN ${this.wrapIdentifier(
        column
      )} SET DEFAULT ${this.wrapLiteral(newDefault.toString())}`;
    }
  }
  ...
  renameColumn(column: string, newName: string) {
    return `RENAME COLUMN ${this.wrapIdentifier(
      column
    )} TO ${this.wrapIdentifier(newName)}`;
  }

  setComment(column: string, newComment: string) {
    return `COMMENT ON COLUMN ${this.tableName}.${this.wrapIdentifier(
      column
    )} IS '${this.escapeString(newComment)}'`;
  }
  ...
  addColumn(item: SchemaItem) {
    ...
    return [
      ...
      item.defaultValue
        ? `DEFAULT ${this.wrapLiteral(item.defaultValue)}`
        : null,
      item.extra,
  ...
  nameFilter = () =>
    this.disabledRename
      ? (i: SchemaItemChange) => i.changeType !== "columnName"
      : () => true;
  alterFilter = () =>
    this.disabledAlter
      ? (i: SchemaItemChange) => i.changeType === "columnName"
  ...
  renames(items: SchemaItemChange[]) {
    return items
      ...
      .filter(this.alterFilter)
      .filter((c) => c.changeType === "columnName")
      .map((item) => {
  ...
  dropIndexes(drops: DropIndexSpec[]): string | null {
    ...
    const names = drops
      .map((spec) => this.dialectData.wrapIdentifier(spec.name))
      .join(",");
  ...
  singleRelation(spec: CreateRelationSpec): string {
    ...
    const fkName = spec.constraintName
      ? this.wrapIdentifier(spec.constraintName)
      : "";
    ...
    const onUpdate = spec.onUpdate
      ? `ON UPDATE ${this.wrapLiteral(spec.onUpdate)}`
      : "";
    const onDelete = spec.onDelete
      ? `ON DELETE ${this.wrapLiteral(spec.onDelete)}`
      : "";
