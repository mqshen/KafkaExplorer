...
const supportsLength = ["char", "varchar"];

const defaultLength = (t: string) => (t.startsWith("var") ? 255 : 8);

export const SqliteData: DialectData = {
  columnTypes: types.map(
    (t) => new ColumnType(t, supportsLength.includes(t), defaultLength(t))
  ),
  ...
  notices: {
    infoSchema:
      "Note: SQLite does not support any column alterations except renaming",
  },
};
