...
export interface DataStoreMutations<T, X extends DataState<T>>
  extends MutationTree<X> {
...
export interface DataStoreActions<T, X extends DataState<T>>
  extends ActionTree<X, RootState> {
...
export interface DataStore<T, X extends DataState<T>>
  extends Module<X, RootState> {
  state: X;
...
export function localActionsFor<T extends ApplicationEntity>(
  cls: any,
  other: any,
  loadOptions: any = {}
) {
    ...
    async poll(context) {
      ...
      await havingCli(context, async (cli) => {
        try {
          ...
          // this is to account for when the store module changes
          const rightItems = items.filter(
            (item) => item.workspaceId === context.rootState.workspaceId
          );
          if (rightItems.length === items.length) {
            context.commit("replace", rightItems);
          }
