import {
  IGroupedUserSettings,
  UserSetting,
  UserSettingValueType,
} from "../../../common/appdb/models/user_setting";
...
const SettingStoreModule: Module<State, any> = {
  ...
  mutations: {
    replaceSettings(state, newSettings: UserSetting) {
      ...
      state.settings = _.mapValues(
        grouped,
        (v) => v[0]
      ) as IGroupedUserSettings;
    },
  ...
  getters: {
    ...
    themeValue(state) {
      ...
      if (
        ["system", "dark", "light"].includes(
          state.settings.theme.value as string
        )
      ) {
