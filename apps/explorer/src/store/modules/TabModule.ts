  ...
  getters: {
    ...
    fatTabs(state, _getters, rootState) {
      return state.tabs.map((tab) => {
        const query = tab.queryId
          ? rootState["data/queries"]["items"].find((q) => q.id === tab.queryId)
          : null;
        const table = tab.tableName
          ? rootState.tables.find((t) => {
              t.name === tab.tableName && t.schema === tab.schemaName;
            })
          : null;
        return {
          tab,
