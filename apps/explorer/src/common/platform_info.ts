...
if (electron.remote) {
  windowPrefersDarkMode = window.matchMedia(
...
if (p.env.PORTABLE_EXECUTABLE_DIR) {
  userDirectory = path.join(
    p.env.PORTABLE_EXECUTABLE_DIR,
    "beekeeper_studio_data"
  );
}
const platformInfo = {
  isWindows,
  ...
  darkMode: testMode
    ? true
    : e.nativeTheme.shouldUseDarkColors || windowPrefersDarkMode,
  userDirectory,
  downloadsDirectory,
  homeDirectory,
  testMode,
  ...
  cloudUrl: isDevEnv
    ? "https://staging.beekeeperstudio.io"
    : "https://app.beekeeperstudio.io",
