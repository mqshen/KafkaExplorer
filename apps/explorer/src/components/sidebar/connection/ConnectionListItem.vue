...
export default {
  ...
  computed: {
    ...
    classList() {
      return {
        active:
          this.savedConnection && this.selectedConfig
            ? this.savedConnection === this.selectedConfig
            : false,
      }
    },
    ...
    savedConnection() {
      if (this.isRecentList) {
        ...
        return this.connectionConfigs.find(
          (c) =>
  ...
  methods: {
    ...
    async click() {
      if (this.savedConnection) {
        ...
        const editable = await this.$store.dispatch(
    ...
    async copyUrl() {
      try {
        ...
        this.$noty.success(
          `The ${this.connectionType} was successfully copied!`
        )
      } catch (err) {
