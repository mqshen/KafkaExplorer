  ...
  <div class="sidebar-wrap row">
    ...
    <div class="tab-content flex-col expand">
      ...
      <div class="connection-wrap expand flex-col">
        ...
        <!-- Saved Connections -->
        <div class="list saved-connection-list expand"
          ...
          <div class="list-group">
              ...
              <div class="flex">
                ...
                <div class="actions">
                  <a v-if="isCloud"
                     ...
                     title="Import connections from local workspace"><i class="material-icons">save_alt</i></a>
                  <a @click.prevent="refresh"><i class="material-icons">refresh</i></a>
                </div>
                <x-button class="actions-btn btn btn-link btn-small"
                          title="Sort By">
                  <!-- <span>{{sortables[this.sortOrder]}}</span> -->
                  <i class="material-icons-outlined">sort</i>
                  <!-- <i class="material-icons">arrow_drop_down</i> -->
                  <x-menu style="--target-align: right;">
                    <x-menuitem v-for="i in Object.keys(sortables)"
                                :key="i"
                                :toggled="i === sortOrder"
                                togglable
            ...
            <div v-else-if="empty"
              ...
              <div class="empty-actions"
                   v-if="isCloud">
                <a class="btn btn-flat btn-block btn-icon"
                   @click.prevent="importFromLocal"
                   title="Import connections from local workspace"><i class="material-icons">save_alt</i> Import</a>
              </div>
            </div>
            <nav v-else
                 class="list-body">
              <sidebar-folder v-for="{ folder, connections } in foldersWithConnections"
                              :key="`${folder.id}-${connections.length}`"
                              :title="`${folder.name} (${connections.length})`"
                              placeholder="No Items"
                              :expandedInitially="true">
                <connection-list-item v-for="c in connections"
                                      :key="c.id"
                                      :config="c"
        ...
        <!-- Recent Connections -->
        <div class="list recent-connection-list expand"
          ...
          <div class="list-group">
            ...
            <nav class="list-body">
              <connection-list-item v-for="c in usedConfigs"
                                    :key="c.id"
                                    :config="c"
                                    :selectedConfig="selectedConfig"
                                    :isRecentList="true"
  ...
  computed: {
    ...
    lonelyConnections() {
      ...
      return this.sortedConnections.filter((config) => {
        return (
          !config.connectionFolderId ||
          !folderIds.includes(config.connectionFolderId)
        )
      })
    },
    foldersWithConnections() {
      ...
      const result = this.folders.map((folder) => {
        return {
          folder,
          connections: this.sortedConnections.filter(
    ...
    components() {
      return [this.$refs.savedConnectionList, this.$refs.recentConnectionList]
    },
  },
  mounted() {
