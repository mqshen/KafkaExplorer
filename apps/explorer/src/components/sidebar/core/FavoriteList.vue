  ...
  <div class="sidebar-favorites flex-col expand">
    <div class="sidebar-list">
      <div class="list-group">
          ...
          <div class="sub row flex-middle expand">
            ...
            <div class="actions">
              <a v-if="isCloud"
                 ...
                 @click.prevent="importFromLocal"><i class="material-icons">save_alt</i></a>
              <a class=""
                 @click.prevent="refresh">
                <i title="Refresh Saved Queries"
        ...
        <nav v-else-if="savedQueries.length > 0"
             class="list-body"
             ref="wrapper">
          <sidebar-folder v-for="({ folder, queries }) in foldersWithQueries"
                          :key="`${folder.id}-${queries.length}`"
                          :title="`${folder.name} (${queries.length})`"
                          :expandedInitially="true">
            <favorite-list-item v-for="item in queries"
                                :key="item.id"
                                :item="item"
          ...
          <favorite-list-item v-for="item in lonelyQueries"
                              :key="item.id"
                              :item="item"
                              :active="isActive(item)"
                              :selected="selected === item"
        ...
        <div class="empty"
          ...
          <span class="empty-actions"
            ...
            <a class="btn btn-flat btn-block btn-icon"
               ...
               title="Import queries from local workspace"><i class="material-icons">save_alt</i> Import</a>
          </span>
        </div>
      </div>
    </div>
    <modal class="vue-dialog beekeeper-modal"
           name="rename-modal"
      ...
      <div class="dialog-content"
        ...
        <query-rename-form :query="renameMe"
  ...
  computed: {
    ...
    foldersWithQueries() {
      return this.folders.map((folder) => {
        return {
          folder,
          queries: this.savedQueries.filter(
