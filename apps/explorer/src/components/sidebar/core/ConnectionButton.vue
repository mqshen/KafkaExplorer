<template>
  <div class="connection-button flex flex-middle"
       v-if="config"
       :title="$bks.buildConnectionString(config)">
    <x-button class="btn btn-link btn-icon"
              menu>
      ...
      <x-menu>
        <x-menuitem @click.prevent="disconnect(false)"
                    class="red">
    ...
    <modal class="vue-dialog beekeeper-modal save-connection-modal"
      ...
      <div class="dialog-content">
        ...
        <SaveConnectionForm :selectInput="true"
      ...
      <form @submit.prevent="disconnect(true)">
        ...
        <div class="vue-dialog-buttons">
          <button class="btn btn-flat"
                  type="button"
                  @click.prevent="$modal.hide('running-exports-modal')">Cancel</button>
  ...
  computed: {
    ...
    connectionName() {
      ...
      const name = config.name
        ? config.name
        : this.$bks.simpleConnectionString(config)
      return name
    },
    connectionType() {
