  ...
  <div class="core-tabs"
    ...
    <div class="tabs-header">
      ...
      <Draggable :options="dragOptions"
        ...
        <core-tab-header v-for="tab in tabItems"
                         :key="tab.id"
                         :tab="tab"
                         ...
                         @duplicate="duplicate"></core-tab-header>
      </Draggable>
      <!-- </div> -->
      <span class="actions">
        <a @click.prevent="createQuery(null)"
           class="btn-fab add-query"><i class=" material-icons">add_circle</i></a>
      </span>
    </div>
    <div class="tab-content">
      ...
      <div v-for="(tab, idx) in tabItems"
        ...
        <div>editor</div>
        <tab-with-table v-if="tab.type === 'table'"
                        :tab="tab"
                        @close="close">
          <template v-slot:default="slotProps">
            <div :table="slotProps.table">tabletable</div>
          </template>
        </tab-with-table>
        <tab-with-table v-if="tab.type === 'table-properties'"
                        :tab="tab"
                        @close="close">
          <template v-slot:default="slotProps">
            <div :table="slotProps.table">tabletable</div>
        ...
        <!-- <TableBuilder v-if="tab.type === 'table-builder'"
  ...
  computed: {
    ...
    rootBindings() {
      return [
        ...
        {
          event: AppEvent.openTableProperties,
          handler: this.openTableProperties,
        },
      ...
      return [
        {
          name: "Close",
          slug: "close",
          handler: ({ item }) => this.close(item),
        },
        {
          name: "Close Others",
          slug: "close-others",
          handler: ({ item }) => this.closeOther(item),
        },
        { name: "Close All", slug: "close-all", handler: this.closeAll },
        {
          name: "Duplicate",
          slug: "duplicate",
          handler: ({ item }) => this.duplicate(item),
        },
      ]
    },
    lastTab() {
  ...
  methods: {
    ...
    async loadTableCreate(table) {
      ...
      if (table.entityType === "table")
        method = this.connection.getTableCreateScript
      if (table.entityType === "view")
        method = this.connection.getViewCreateScript
      if (!method) {
        this.$noty.error(
          `Can't find script for ${table.name} (${table.entityType})`
        )
        return
      }
    },
    async loadRoutineCreate() {},
    openTableBuilder() {
