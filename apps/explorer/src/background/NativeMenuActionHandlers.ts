...
function getIcon() {
  ...
  return path.resolve(
    path.join(__dirname, "..", `${iconPrefix}/icons/png/512x512.png`)
  );
}
...
export default class NativeMenuActionHandlers implements IMenuActionHandler {
  ...
  about() {
    app.setAboutPanelOptions({
      applicationName: "Kafka Explorer",
      applicationVersion: app.getVersion(),
      copyright: "Kafka Explorer Team",
      authors: ["mqshen"],
  ...
  opendocs() {
    shell.openExternal("https://docs.github.io/");
  }
  ...
  addBeekeeper = async (_1: Electron.MenuItem, win: ElectronWindow) => {
    const existing = await SavedConnection.findOne({
      where: { defaultDatabase: platformInfo.appDbPath },
    });
    if (!existing) {
