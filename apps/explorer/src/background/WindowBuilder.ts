...
class BeekeeperWindow {
  ...
  constructor(settings: IGroupedUserSettings, openOptions?: OpenOptions) {
    ...
    const dark =
      ...
      theme.value.toString().includes("dark");
    const showFrame =
      settings.menuStyle && settings.menuStyle.value == "native" ? true : false;
    const titleBarStyle =
      platformInfo.isWindows && settings.menuStyle.value == "native"
        ? "default"
        : "hidden";
    ...
    if (
      (platformInfo.env.development && !platformInfo.env.test) ||
      platformInfo.debugEnabled
    ) {
