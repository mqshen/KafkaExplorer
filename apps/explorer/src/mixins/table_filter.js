  ...
  methods: {
    filter(rawTables, allFilters) {
      const tables = rawTables.filter((table) => {
        return (
          (table.entityType === "table" && allFilters.showTables) ||
          (table.entityType === "view" && allFilters.showViews) ||
          (table.entityType === "materialized-view" && allFilters.showViews) ||
          (Object.keys(RoutineTypeNames).includes(table.type) &&
            allFilters.showRoutines)
        );
      });
      ...
      const startsWithFilter = _(tables)
        .filter((item) =>
          _.startsWith(item.name.toLowerCase(), filterQuery.toLowerCase())
        )
        .value();
