...
export default async function (server, database) {
  ...
  return {
    ...
    selectTop: (table, offset, limit, orderBy, filters) =>
      selectTop(conn, table, offset, limit, orderBy, filters),
    selectTopStream: (db, table, orderBy, filters, chunkSize) =>
      selectTopStream(conn, db, table, orderBy, filters, chunkSize),
    getInsertQuery: (tableInsert) =>
      getInsertQuery(conn, database.database, tableInsert),
...
export async function getTableLength(conn, table) {
  ...
  const countResults = await driverExecuteQuery(conn, {
    query: countQuery,
    params,
  });
...
export async function selectTopStream(
  conn,
  db,
  table,
  orderBy,
...
export async function listTableColumns(conn, database, table) {
  ...
  const final = _.flatMap(results, (item, idx) =>
    dataToColumns(item.result.data, item.tableName)
  );
  return final;
}
...
export async function listTableIndexes(conn, database, table) {
  ...
  const allSQL = data
    .map((row) => `PRAGMA INDEX_XINFO('${escapeString(row.name)}')`)
    .join(";");
  const infos = await driverExecuteQuery(conn, {
    query: allSQL,
    multiple: true,
  });
...
export function driverExecuteQuery(conn, queryArgs) {
  const runQuery = (connection, { text }) =>
